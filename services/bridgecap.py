...
def ussd_handler(message: IncomingMessageSchema, session_id, cfg=None):
    ...
    if 'c.us' in message.chatId:
        if '254742976667' in message.chatId:
            print("modifying url...")
            url = f'{cfg["bridgecap"]["remote_url"]}/ussd?MSISDN=0712345678&session_id={session_id}&ussd_string={body}'
        else:
            url = f'{cfg["bridgecap"]["remote_url"]}/ussd?MSISDN={message.chatId.replace("@c.us", "")}&session_id={session_id}&ussd_string={body}'
